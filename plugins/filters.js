...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
    ...
    if(banned !== undefined) return
    if (!!message.mention && message.mention[0] == '917356727986@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./stickers/Akku.webp'), MessageType.sticker, { mimetype: Mimetype.webp, quoted : message.data, ptt: false})
    }
const array = ['love','list','machu','menu','mute','onn','oo','so','unmute','wait','adi','army','bye','chaya','edit','ip','kayari','mari','money','monu','nallath','night','nyt','on','patti','po','poth','sir','Amal','Hii','ee','Da','food','Hlo','para','love u','set','sed','jada','amal','Hi','sad','don','Loki','Ayyapan','Chinnadan','Poda','Love','Eee','SACHU FF','Bad boy','Akkubro','Ok bye','@','bye']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
